...
(function () {
  ...
  async function getBookedTimes(date, master) {
    const response = await fetch(`${SUPABASE_URL}/rest/v1/appointments?select=booking_time,master&salon_id=eq.${encodeURIComponent(NAILS_HERE_SALON_ID)}&booking_date=eq.${encodeURIComponent(date)}&status=neq.cancelled`, {
    ...
    rows.forEach(row => {
      ...
      byTime.get(time).push(row.master || '');
    });
    for (const [time, masters] of byTime) {
      if (master === 'Любой свободный') {
        if (masters.includes('Любой свободный') || MASTERS.every(m => masters.includes(m))) booked.add(time);
      } else if (masters.includes(master) || masters.includes('Любой свободный')) {
        booked.add(time);
      }
    }
    return booked;
  ...
  async function renderTimes() {
    ...
    try {
      ...
      if (!timesList.querySelector('.booking-time-item:not(.booking-time-busy)')) {
        const note = document.createElement('div');
        note.className = 'booking-times-placeholder booking-no-slots';
        note.textContent = 'На эту дату свободных окон нет. Выберите другую дату или мастера.';
        timesList.appendChild(note);
      }
    } catch (error) {
      console.error(error);
      timesList.innerHTML = '<div class="booking-times-placeholder">Не удалось загрузить расписание. Попробуйте ещё раз.</div>';
    }
  }

  widget.querySelectorAll('input[name="service"]').forEach(checkbox => checkbox.addEventListener('change', () => {
    state.services = Array.from(widget.querySelectorAll('input[name="service"]:checked')).map(el => ({ name: el.value, price: parseInt(el.dataset.price,10) }));
    validateStep();
  }));

  widget.querySelectorAll('input[name="master"]').forEach(radio => radio.addEventListener('change', async () => {
    state.master = radio.value;
    state.time = null;
    if (state.date) await renderTimes();
    validateStep();
  }));
  ...
  function validateStep() {
    ...
    else valid = nameInput.value.trim() !== '' && phoneInput.value.trim() !== '';
  ...
  function renderSummary() {
    ...
    summaryBox.innerHTML = `<div class="booking-summary-row"><span class="booking-summary-label">Услуги</span><span>${state.services.map(s => s.name).join(', ')}</span></div><div class="booking-summary-row"><span class="booking-summary-label">Мастер</span><span>${state.master}</span></div><div class="booking-summary-row"><span class="booking-summary-label">Дата и время</span><span>${state.date.label}, ${state.time}</span></div><div class="booking-summary-row"><span class="booking-summary-label">Итого</span><span>${total.toLocaleString('ru-RU')} ₽</span></div>`;
  }

  function goToStep(step) {
    currentStep = step;
    panels.forEach(panel => panel.classList.toggle('active', parseInt(panel.dataset.step,10) === step));
    stepIndicators.forEach(indicator => { const n=parseInt(indicator.dataset.stepIndicator,10); indicator.classList.toggle('active',n===step); indicator.classList.toggle('done',n<step); });
    ...
    if (step !== TOTAL_STEPS) { successBox.classList.remove('active'); successBox.style.display='none'; }
    else { successBox.classList.remove('active'); successBox.style.display='none'; const fields=widget.querySelector('.booking-form-fields'); if(fields) fields.style.display='flex'; summaryBox.style.display='block'; }
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    submitBtn.textContent = 'Проверяем свободное окно…';
    try {
      const booked = await getBookedTimes(state.date.iso, state.master);
      if (booked.has(state.time)) {
        state.time = null;
        await renderTimes();
        submitBtn.textContent = 'Отправить заявку';
        validateStep();
        alert('Это время уже занято. Пожалуйста, выберите другое свободное окно.');
        return;
      }
      ...
      if (!response.ok) {
        ...
        if (errorText.includes('SLOT_ALREADY_BOOKED') || errorText.includes('appointments_unique_active_slot_idx')) throw new Error('Это время только что заняли. Пожалуйста, выберите другое свободное окно.');
        throw new Error(errorText || `HTTP ${response.status}`);
      }
      const fields=widget.querySelector('.booking-form-fields'); if(fields) fields.style.display='none';
      summaryBox.style.display='none'; submitBtn.style.display='none'; backBtn.style.display='none'; successBox.style.display='block'; successBox.classList.add('active'); restartBtn.style.display='block'; stepIndicators.forEach(indicator=>indicator.classList.add('done'));
      const successText=successBox.querySelector('p'); if(successText) successText.textContent='Спасибо! Ваша заявка успешно отправлена. Мы свяжемся с вами для подтверждения записи.';
    } catch(error) {
      console.error(error);
      alert('Не удалось оформить запись:\n\n' + error.message);
      submitBtn.disabled=false; submitBtn.textContent='Отправить заявку';
    }
  });

  restartBtn.addEventListener('click', () => {
    state.services=[]; state.master=null; state.date=null; state.time=null;
    ...
    datesList.innerHTML=''; timesList.innerHTML='<div class="booking-times-placeholder">Сначала выберите дату</div>'; nameInput.value=''; phoneInput.value='';
    const fields=widget.querySelector('.booking-form-fields'); if(fields) fields.style.display='flex';
    summaryBox.style.display='block'; successBox.classList.remove('active'); successBox.style.display='none'; submitBtn.textContent='Отправить заявку'; submitBtn.disabled=true; goToStep(1);
  });
  goToStep(1);
})();
